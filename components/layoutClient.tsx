...
const Navbar = () => (
  <nav
    className="fixed top-0 left-0 right-0 z-40 h-8 border-b border-separator bg-background/70 backdrop-blur-lg"
  ...
  >
    <header className="flex h-full items-center justify-between px-4 sm:px-6 lg:px-8">
      ...
      <p className="text-inherit font-bold">Home</p>
      <ThemeSwitch />
    </header>
  </nav>
);
  ...
  return (
    ...
    >
      <button
        ...
        onClick={onToggle}
      >
        <ChevronLeft className={clsx("h-3 w-3 transition-transform", isCollapsed && "rotate-180")} />
      </button>
      <nav className="flex flex-col gap-1 p-3 pt-4">
        {items.map((item) => {
          const isActive = pathname === item.href;
          const Icon = item.icon ?? SquareDashed;
          return (
            <NextLink
              key={item.href}
              href={item.href}
